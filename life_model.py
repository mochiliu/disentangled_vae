...
class VAE(object):
  ...
  def _create_recognition_network(self, x, reuse=False):
    with tf.variable_scope("rec", reuse=reuse) as scope:
      ...
      h_conv2 = tf.nn.relu(self._conv2d(h_conv1,    W_conv2, 2) + b_conv2) # (8, 8)
      h_conv3 = tf.nn.relu(self._conv2d(h_conv2,    W_conv3, 2) + b_conv3) # (4, 4)
      h_conv3_flat = tf.reshape(h_conv3, [-1, 4*4*32])
  ...
  def _create_generator_network(self, z, reuse=False):
    with tf.variable_scope("gen", reuse=reuse) as scope:
      ...
      W_deconv3, b_deconv3 = self._conv2d_weight_variable([4, 4,  1, 32], "deconv3", deconv=True)
      ...
      h_deconv3   =            self._deconv2d(h_deconv2,      W_deconv3, 16, 16, 2) + b_deconv3
      #h_deconv4   =            self._deconv2d(h_deconv3,      W_deconv4, 32, 32, 2) + b_deconv4
      
      x_out_logit = tf.reshape(h_deconv3, [-1, 32*32*1])
      return x_out_logit

    
  def _create_network(self):
    # tf Graph input
    self.x = tf.placeholder(tf.float32, shape=[None, 32*32])
